<script type="text/javascript">
    ...
    var lang_push = function (langs) {
        Object.assign(SCRIPT_LANG, langs);
    };
    ...
    VnSapp.config(function (NotificationProvider, $stateProvider, $urlRouterProvider, cfpLoadingBarProvider, $sceProvider, $provide, $httpProvider) {
        ...
        $stateProvider
            ...
            @php ($angularState = get_angular_state())
            @foreach($angularState as $state)
        .state('{{ $state['name'] }}', {
            url: '{{ $state['url'] }}',
    ...
    VnSapp.run(function ($rootScope, $state, $stateParams, $http, $timeout, $window, $document, $uibModal, $location, Dialog, clipboard, Notification, Thumbs) {
        ...
        $rootScope.viewPost = function (row) {
            $uibModal.open({
                ...
                controller: function ($rootScope, $scope, $uibModalInstance) {
                    $scope.url = '{{url(config('site.uri.post'))}}';
                    for (key in row) {
                        $scope.url = $scope.url.replace('{'+key+'}', row[key]);
                    }
        ...
        $rootScope.postUrl = function (row) {
            var url = '{{url(config('site.uri.post'))}}';
            for (key in row) {
        ...
        $rootScope.viewCat = function (row) {
            $uibModal.open({
                animation: true,
                templateUrl: 'iframe.html',
                controller: function ($rootScope, $scope, $uibModalInstance) {
                    $scope.url = '{{url(config('site.uri.category'))}}';
                    for (key in row) {
                        $scope.url = $scope.url.replace('{'+key+'}', row[key]);
                    }
                    $scope.close = function () {
        ...
        $rootScope.catUrl = function (row) {
            var url = '{{url(config('site.uri.category'))}}';
            for (key in row) {
                url = url.replace('{'+key+'}', row[key]);
            }
            clipboard.copyText(url);
    ...
    VnSapp.constant('Thumbs', {
        ...
        noImage: IMAGE_PATH + 'icons/noImage.png'
    });
    ...
    VnSapp.filter('parseObj', function () {
        return function (input, key, glue) {
            if (key == undefined) {
                key = 'name';
            }
            if (glue == undefined) {
    ...
    VnSapp.filter('parseImage', function (Thumbs) {
        return function (input) {
            if(input == '' || input == null || input == false) {
                return Thumbs.noImage;
            }
            return input.replace(/([0-9]{4})\/([0-9]{2})\/([0-9a-z]{32})\.(jpg|png)/, '$1/$2/thumbs/$3.$4');
        };
    });
    ...
    VnSapp.filter('translate', function () {
        return function (trans, replaces) {
            return __(trans, replaces);
        }
    });
        ...
        return function (baseParams) {
            self.params = {
                ...
                breadcrumb: {
                    ...
                    root: {
                        id: 0,
                        name: '{{__('Root breadcrumb')}}'
                    }
                }
            };

            if (baseParams.breadcrumb != undefined) {
                Object.assign(self.params.breadcrumb, baseParams.breadcrumb);
                delete baseParams.breadcrumb;
            } else {
                delete self.params.breadcrumb;
            }

            Object.assign(self.params, baseParams);
            ...
            return {
                ...
                getColumns: function () {
                    for (field in self.params.columns) {
                        if (self.params.columns[field].order) {
                            this.reOrderUrl = self.params.columns[field].order;
                            self.params.columns[field].type = 'order';
                        }
                    }
                    return self.params.columns;
                },
                breadcrumbInit: function () {
                    if (self.params.breadcrumb != undefined) {
                        this.breadcrumbs = [
                            {
                                key: self.params.breadcrumb.root[self.params.breadcrumb.key],
                                label: self.params.breadcrumb.root[self.params.breadcrumb.label]
                            }
                        ];
                        this.filter[self.params.breadcrumb.filter] = self.params.breadcrumb.root[self.params.breadcrumb.key];
                    }
                },
                breadcrumbClick: function (index) {
                    this.breadcrumbs.splice(index+1);
                    this.filter[self.params.breadcrumb.filter] = this.breadcrumbLastKey();
                    this.reload();
                },
                breadcrumbAdd: function (row) {
                    this.breadcrumbs.push({key: row[self.params.breadcrumb.key], label: row[self.params.breadcrumb.label]});
                    this.filter[self.params.breadcrumb.filter] = row[self.params.breadcrumb.key];
                    this.reload();
                },
                breadcrumbLast: function () {
                    return this.breadcrumbs[this.breadcrumbLastIndex()];
                },
                breadcrumbLastKey: function () {
                    return this.breadcrumbLast().key;
                },
                breadcrumbLastLabel: function () {
                    return this.breadcrumbLast().label;
                },
                breadcrumbLastIndex: function () {
                ...
                url: function () {
                    ...
                    if(Object.keys(this.sorting).length > 0) {
                        for (key in this.sorting) {
                            url['sorting['+key+']'] = this.sorting[key];
                        }
                    }
                    return url;
                }
    ...
    VnSapp.controller('vnsTableCtrl', function ($rootScope, $scope, $http, Notification) {
        ...
        $scope.sortInit = function () {
            ...
            for (field in $scope.$columns) {
                ...
                } else if ($scope.$columns[field].order != undefined) {
                    sorting[field] = 'asc';
                }
            }
        ...
        $scope.makePages = function () {
            var range = [];
            for(var i=1;i<=$scope.params.totalPage;i++) {
                range.push(i);
            }
            return range;
        };
        ...
        $scope.$treeOptions = {
            dropped : function (e) {
                if ($scope.params.reOrderUrl != undefined) {
                    var reformattedArray = $scope.$rows.map(function (obj) {
                        return obj.id;
                    });

                    $http.post($scope.params.reOrderUrl, {'data': reformattedArray}).then(function (respone) {
        ...
        $scope.$counts = $scope.params.counts;
        $scope.sortInit();
        $scope.params.breadcrumbInit();
        $scope.params.reload();

        $scope.$watch('params.count', function (newValue) {
            $scope.$count = newValue;
        });

        $scope.$watch('params.breadcrumbs', function (newValue) {
            $scope.$breadcrumbs = newValue;
        });

        $scope.$watch('params.total', function (newValue) {
            $scope.$total = newValue;
        });

        $scope.$watch('params.rows', function (newValue) {
            $scope.$rows = newValue;
        });

        $scope.$watch('params.page', function () {
            ...
            columns: {
                ...
                displayName: {
                    ...
                    and: {
                        ...
                        authors: {
                            format: function (value) {
                                return '- {{__('by')}} '+$filter('parseObj')(value);
                            },
                            text: 'muted',
                            size: 'small'
                        },
            ...
            $uibModal.open({
                ...
                controller: function ($scope, $uibModalInstance) {
                    ...
                    $scope.activate = function () {
                        ...
                        $module.$update(function (response) {
                            tableParams.reload();
                            Notification.success('{{__('Saved successfully')}}');
                        }, function (xhr) {
                            if (xhr.status == 422) {
    ...
    VnSapp.controller('ThemeCtrl', function ($rootScope, $scope, $uibModal, $filter, vnsTableParams, $resource, Dialog, Notification, $http) {
            ...
            columns: {
                ...
                displayName: {
                    ...
                    and: {
                        ...
                        default: {
                            format: function (value) {
                                return value == true?'<span class="label label-success">{{__('Default')}}</span>':'';
                            },
                            text: 'muted',
                            size: 'small'
                        },
        ...
        $scope.gadget = function () {
            $uibModal.open({
                animation: true,
                templateUrl: 'theme/gadget.html',
                backdrop: 'static',
                controller: function ($scope, $uibModal, $uibModalInstance, $http) {
                    $http.get(API_URL + '/theme/gadget').then(function (response) {
                        $scope.$name = response.data.theme;
                        $scope.$groups = response.data.groups;
                        $scope.$gadgets = response.data.gadgets;
                        $scope.$themeGadgets = response.data.themeGadgets;
                    });

                    $scope.add = function (groupKey) {
                        $uibModal.open({
                            animation: true,
                            templateUrl: 'theme/gadget/add.html',
                            resolve: {
                                gadgets: $scope.$gadgets,
                                groups: $scope.$themeGadgets
                            },
                            controller: function ($scope, $uibModalInstance, gadgets, groups) {
                                $scope.$gadgets = angular.copy(gadgets);
                                $scope.$gadget = {};
                                $scope.init = function(gadgetKey){
                                    $scope.$gadget = {
                                        name: gadgetKey,
                                        visible: true
                                    };
                                    if($scope.$gadgets[gadgetKey].parameters != undefined) {
                                        $scope.$gadget.parameters = {};
                                        $scope.$parameters = $scope.$gadgets[gadgetKey].parameters;
                                        for(key in $scope.$gadgets[gadgetKey].parameters) {
                                            $scope.$gadget.parameters[key] = null;
                                        }
                                    }
                                };
                                $scope.save = function () {
                                    groups[groupKey].push($scope.$gadget);
                                    $uibModalInstance.dismiss('close');
                                };
                                $scope.close = function () {
                                    $uibModalInstance.dismiss('close');
                    ...
                    $scope.edit = function (gadget) {
                        $uibModal.open({
                            animation: true,
                            templateUrl: 'theme/gadget/edit.html',
                            backdrop: 'static',
                            resolve: {
                                parameters: $scope.$gadgets[gadget.name].parameters,
                                gadgetName: function () {
                                    return $scope.$gadgets[gadget.name]!=undefined?$scope.$gadgets[gadget.name].name:gadget.name;
                                }
                            },
                            controller: function ($scope, $uibModalInstance, gadgetName, parameters) {
                                $scope.$name = gadgetName;
                                $scope.$gadget = angular.copy(gadget);
                                $scope.$parameters = angular.copy(parameters);
                                $scope.save = function () {
                                    gadget.visible = $scope.$gadget.visible;
                                    gadget.parameters = $scope.$gadget.parameters;
                                    $uibModalInstance.dismiss('close');
                                };
                                $scope.close = function () {
                                    $uibModalInstance.dismiss('close');
                    ...
                    $scope.toggleVisible = function (gadget) {
                        gadget.visible = !gadget.visible;
                    };

                    $scope.delete = function (groupKey, key) {
                        Dialog.confirm(__('Are you sure you want to delete <b>:name</b>?', {name: $scope.$gadgets[$scope.$themeGadgets[groupKey][key].name]!=undefined?$scope.$gadgets[$scope.$themeGadgets[groupKey][key].name].name:$scope.$themeGadgets[groupKey][key].name}))
                            .result.then(function () {
                            $scope.$themeGadgets[groupKey].splice(key,1);
                        });
                    };

                    $scope.save = function () {
                        Dialog.confirm(__('Are you sure you want to save?'))
                            .result.then(function () {
                            $http.post(API_URL + '/theme/gadget', {theme: $scope.$name, gadgets: $scope.$themeGadgets}).then(function (response) {
                                if (response == false) {
                                    Notification.error('{{__('Save failed')}}');
                                } else {
                                    Notification.success('{{__('Saved successfully')}}');
                                }
                            });
            ...
            $uibModal.open({
                ...
                controller: function ($scope, $uibModalInstance) {
                    ...
                    $scope.default = function () {
                        ...
                        $theme.$update(function (response) {
                            tableParams.reload();
                            Notification.success('{{__('Saved successfully')}}');
                        }, function (xhr) {
                            if (xhr.status == 422) {
